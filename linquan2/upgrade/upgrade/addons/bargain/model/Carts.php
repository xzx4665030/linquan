	...
	public function submitByVirtual($carts,$orderSrc = 0){
		...
		try{
			...
			if(false !== $result){
				...
				//修改砍价数量
				Db::name('bargains')->where('bargainId',$carts['bargainId'])->setInc('orderNum',$goods['cartNum']);
				Db::name('bargains')->where('bargainId',$carts['bargainId'])->setDec('goodsStock',$goods['cartNum']);
				//添加订单号
