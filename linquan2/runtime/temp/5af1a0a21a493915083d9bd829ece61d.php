...
<div class="wst-wrap">
  ...
  <div class="wst-shop-head"><span>满减送列表</span></div>
	   ...
	   <tbody id='loadingBdy'>
	      <?php if(is_array($list) || $list instanceof \think\Collection): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lis): $mod = ($k % 2 );++$k;?>
			<tr class='head' style="background-color:#eee">
				...
				<td><a href="<?php echo url('home/shops/edit_delivery',array('id'=>$lis['discount_id'])); ?>">编辑</a>|<a href="<?php echo url('home/shops/delete_delivery',array('id'=>$lis['discount_id'])); ?>">删除</a></td>
			</tr>
		  <?php endforeach; endif; else: echo "" ;endif; ?>
       </tbody>
		<?php endif; ?>
...
function finisedByPage(){
	...
	if(content){
		...
		window.location.href = WST.U('home/shops/delivery');
	}
	
}

function loadStat(){
	window.location.href = WST.U('home/shops/add_promotion','flag=2');
}
</script>
