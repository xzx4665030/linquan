...
<div class="wst-wrap">
  ...
  <div class="wst-shop-head"><span>优惠券列表</span></div>
  ...
  <div class='wst-shop-content'>
    <table class='wst-order-list'>
	   ...
	   <tbody id='loadingBdy'>
	      <?php if(is_array($list) || $list instanceof \think\Collection): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lis): $mod = ($k % 2 );++$k;?>
			<tr class='head' style="background-color:#eee">
				<td><?php echo $k; ?></td>
				<td><?php echo $lis['discount_name']; ?></td>
				<td><?php echo $lis['discount_num']; ?></td>
				<td><?php echo $lis['discount_consume']; ?></td>
				<td><?php echo $lis['discount_value']; ?></td>
				<td><?php echo date("Y-m-d",$lis['discount_startDate']); ?>—<?php echo date("Y-m-d",$lis['discount_endDate']); ?></td>
				<td><a href="<?php echo url('home/shops/edit_promotion',array('id'=>$lis['discount_id'])); ?>">编辑</a>|<a href="<?php echo url('home/shops/delete_promotion',array('id'=>$lis['discount_id'])); ?>">删除</a></td>
			</tr>
		  <?php endforeach; endif; else: echo "" ;endif; ?>
       </tbody>
		<?php endif; ?>
...
function finisedByPage(){
	...
	if(content){
		data = {"data":content};
		$.ajax({
			url:WST.U('home/shops/promotion'),
			data:data,
			type:'post',
			dataType:'json',
			success:function(e){
				...
				for(var k=0;k<e.length;k++){						
					...
					html_tr=html_tr+'<td>'+e[k].discount_consume+'</td>';
		...
		window.location.href = WST.U('home/shops/promotion');
	}
	
}

function loadStat(){
	window.location.href = WST.U('home/shops/add_promotion','flag=1');
}
</script>
