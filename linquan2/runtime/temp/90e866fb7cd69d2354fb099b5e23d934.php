...
<div class="wst-wrap">
  ...
  <div class="wst-shop-head"><span>添加优惠券</span></div>
  
	<form method="post" action="<?php echo url('home/shops/save_promotion'); ?>" enctype="multipart/form-data">
		优惠券有效期：<input type='text' class="laydate-icon j-ipt" id='startDate' onclick="laydate()" value='<?php echo $startDate; ?>' name="startDate"/>至
		<input type='text' class="laydate-icon j-ipt" id='endDate' onclick="laydate()" value='<?php echo $endDate; ?>' name="endDate"/><br />
		优惠券名称：<input type="text" name="promotion_name"><br />
		优惠券数量：<input type="text" name="promotion_num"><br />
		最低消费：	 <input type="text" name="promotion_consume"><br />
		优惠金额：<input type="text" name="promotion_value"><br />
		优惠券图片：
		...
		<img src="" id="show_img">
		<input type="hidden" name="promotion_img" id="promotion_img">
		<br />
