...
<div class="wst-container">
    ...
    <div class="pay-sbox-head">
    	<a href='<?php echo \think\Request::instance()->root(true); ?>'>首页</a>&nbsp;&gt;&nbsp;支付
    </div>
    <div>
	    ...
	    <div>
		    <div id="wst-orders-box" style="display:none;">
			    <table class='wst-order-list'>
				 	<?php if(is_array($rs['list']) || $rs['list'] instanceof \think\Collection): $i = 0; $__LIST__ = $rs['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?>
				 	<thead>
					 	<tr>
				   ...
				   <tbody class="otbody">
				 	<?php if(is_array($rs['goods'][$order['orderId']]) || $rs['goods'][$order['orderId']] instanceof \think\Collection): $i = 0; $__LIST__ = $rs['goods'][$order['orderId']];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				 	<tr>
				     	<td class="wst-left-noborder" width="64">
				     		<div class="goods-img">
					     		<a href="<?php echo url('home/goods/detail',array('id'=>$vo['goodsId'])); ?>" target="_blank">
					     			<img class='goodsImg' data-original="__ROOT__/<?php echo $vo['goodsImg']; ?>"  title="<?php echo $vo['goodsName']; ?>" width='60'/>
					     		</a>
				     		</div>
				     	</td>
				        <td class="wst-align-left">
				        	<div><?php echo $vo['goodsName']; if(count($vo['goodsSpecNames']) > 0): ?>
				        		【
				        		<?php if(is_array($vo['goodsSpecNames']) || $vo['goodsSpecNames'] instanceof \think\Collection): $i = 0; $__LIST__ = $vo['goodsSpecNames'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$spec): $mod = ($i % 2 );++$i;?>
				        		<?php echo $spec; ?>&nbsp;
				        		<?php endforeach; endif; else: echo "" ;endif; ?>
				        		】
				        	<?php endif; ?>
		    ...
		    <div class="pay-list">
		    	...
		    	<?php if(is_array($rs['payments']) || $rs['payments'] instanceof \think\Collection): $i = 0; $__LIST__ = $rs['payments'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$payment): $mod = ($i % 2 );++$i;if($payment['isOnline'] == 1): ?>
