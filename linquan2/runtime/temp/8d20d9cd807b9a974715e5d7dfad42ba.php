<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:62:"D:\phpStudy\WWW\linquan2/wstmart/admin\view\mobile\item_6.html";i:1494409386;s:53:"D:\phpStudy\WWW\linquan2/wstmart/admin\view\base.html";i:1490867328;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="utf-8">
...
<div class="waike">
	<table border="" cellspacing="" cellpadding="">
		<tr>
			<td  class="tiaoxuan">
				<p style="height: 20px;line-height: 20px;font-size: 16px;margin-bottom: 15px;">添加店铺版块</p>
       	...
       	console.log(data);
             	if(data.item_data.item){
	           	...
	           	for(var h=0;h<data.item_data.item.length;h++){
		               var jj=jj+'<div class="ggk"><span class="sp_id">'+data.item_data.item[h].shopId+'</span><span class="shanchua">删除</span><div class="img_x"><img src="'+data.item_data.item[h].shopImg+'"/></div>';
    									jj=jj+'<div class="mingzi">'+data.item_data.item[h].shopName+'</div></div>';
...
$(document).ready(function() {
  ...
  $('#serch').click(function(){
    var content=$('#content').val();
		$.ajax({
            type: "post",
             url: WST.U('admin/mobile/get_shop'),
...
//           	console.log(data);
               if(data!=""){
               	...
               	for(var m=0;m<data.length;m++){
               		jj=jj+'<div class="xuanze"><span class="id_b">'+data[m].shopId+'</span><div class="img_b"><img src="'+data[m].shopImg+'" /></div>';
               		jj=jj+'<div class="mingzi_b">'+data[m].shopName+'</div><div class="jiage_b"></div><a class="tianjia_b">添加</a><div style="clear: both;"></div></div>'
               	}
               	$("#xuanze").html(jj);
               }
            }
    ...
    $("#xuanze").on("click",".tianjia_b",function(){
    	...
    	hh=hh+'<div class="mingzi">'+$(this).siblings(".mingzi_b").html()+'</div></div>';
    	$("#tupian").append(hh);
    })
    ...
    $('.tijiao').click(function(){
    	var ids=$("#cashId").val();
      		...
      		if($('.ggk').length>0){
      			ss=ss+',"item":[{"image":"'+$('.ggk').eq(0).find("img").attr('src')+'","shopId":"'+$('.ggk').eq(0).find(".sp_id").html()+'"}';
      			for(var i=1;i<$('.ggk').length;i++){
      				ss=ss+',{"image":"'+$('.ggk').eq(i).find("img").attr('src')+'","shopId":"'+$('.ggk').eq(i).find(".sp_id").html()+'"}';
      			}
      			ss=ss+"]";
