...
class Carts extends Base{
	...
	//判断是否可以添加到购物车  xzx
	public function is_addCart($goodsId,$goodsSpecId,$cartNum,$cart_number){
		//验证传过来的商品是否合法
		$chk = $this->checkGoodsSaleSpec($goodsId,$goodsSpecId);
		if($chk['data']['stock']< ($cartNum+$cart_number)){
			return WSTReturn("不允许加入购物车，购物车数量加购买数量超过商品库存", -1);
		}else{
			return WSTReturn("允许加入购物车", 1);
		}
	}
