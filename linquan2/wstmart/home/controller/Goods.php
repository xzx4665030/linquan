...
class Goods extends Base{
	...
	*/
	public function get_infos(){
		...
		foreach($specIds as $v){
			$where1['itemId'] = $v;
			$spec[] = Db::name('spec_items')->alias('si')->join('spec_cats sc','si.catId=sc.catId')->where($where1)->field('itemName,catName')->find();
		}
		...
		$where2['dataFlag'] = 1;
		$where2['shopStatus'] = 1;
		$shop_info = Db::name('shops')->where($where2)->field('shopName,shopkeeper')->find();
		
		$new_arr['shopkeeper'] = $shop_info['shopkeeper'];
		$new_arr['shopName'] = $shop_info['shopName'];
		$new_arr['spec'] = $spec;
		$new_arr['goodsId'] = $shop_list['goodsId'];
		$new_arr['shopId'] = $shop_list['shopId'];
		$new_arr['goodsName'] = $shop_list['goodsName'];
	...
	*/
    public function operate(){
		...
		$ylf_sl = $_POST['ylf_sl'];   //数量
		$leib = $_POST['leib'];
		if($leib == '1'){
			$data['buhuo_shopId'] = $shopId;
			$data['buhuo_goodsId'] = $goodsId;
			$data['buhuo_num'] = $ylf_sl;
			$data['buhuo_remark'] = $beizhu;
			$data['buhuo_status'] = 1;
			$data['buhuo_time'] = time();
			$res = Db::name('shop_buhuo')->insert($data);
			if($res){
				$datas['flag'] = 1;
				$datas['msg'] = "补货提交成功，请等待";
			}else{
				$datas['flag'] = -1;
				$datas['msg'] = "补货提交失败";
			}
		}else if($leib == '2'){
			$data['tuihuo_shopId'] = $shopId;
			$data['tuihuo_goodsId'] = $goodsId;
			$data['tuihuo_num'] = $ylf_sl;
			$data['tuihuo_remark'] = $beizhu;
			$data['tuihuo_status'] = 1;
			$data['tuihuo_time'] = time();
			$res = Db::name('shop_tuihuo')->insert($data);
			if($res){
				$datas['flag'] = 1;
				$datas['msg'] = "退货提交成功，请等待";
			}else{
				$datas['flag'] = -1;
				$datas['msg'] = "退货提交失败";
			}
