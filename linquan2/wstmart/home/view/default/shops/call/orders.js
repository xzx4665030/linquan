...
function waitDivleryByPage(p){
	...
	params.page = p;
	//console.log(params)
	$.post(WST.U('home/call/waitDeliveryByPage'),params,function(data,textStatus){
		$('#loading').hide();
	    var json = WST.toJson(data);
	    $('.j-order-row').remove();
	    if(json.status==1){
...
function deliveredByPage(p){
  ...
  params.key = $.trim($('#key').val());
  params.page = p;
  $.post(WST.U('home/call/deliveredByPage'),params,function(data,textStatus){
    $('#loading').hide();
      var json = WST.toJson(data);
      $('.j-order-row').remove();
      if(json.status==1){
...
function finisedByPage(p){
	...
	params.key = $.trim($('#key').val());
	params.page = p;
	$.post(WST.U('home/call/finishedByPage'),params,function(data,textStatus){
		$('#loading').hide();
	    var json = WST.toJson(data);
	    $('.j-order-row').remove();
	    if(json.status==1){
...
function view(id){
	location.href=WST.U('home/call/view','id='+id);
}
function viewS(id){
	location.href=WST.U('home/call/viewS','id='+id);
}
