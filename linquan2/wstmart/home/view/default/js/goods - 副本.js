...
function addCart(type,iptId){
	...
	console.log(buyNum+','+goodsInfo.id+','+goodsSpecId);   //lf  判断商品详情页购买数量是否大于库存
	//判断当购物车中数量加购买数量大于库存数量时不允许点击添加购物车的操作  xzx
	$.post(WST.U('home/carts/is_addCart'),{goodsId:goodsInfo.id,goodsSpecId:goodsSpecId,buyNum:buyNum,rnd:Math.random()},function(data,textStatus){
	     var json = WST.toJson(data);
	     if(json.status==1){   //允许加入购物车
