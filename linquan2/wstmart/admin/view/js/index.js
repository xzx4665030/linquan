$(window).resize(function(){
    ...
    $('.wst-accordion').each(function(){
    	liger.get($(this).attr('id')).setHeight(h-28);
...
function initTabMenus(menuId){
	...
	$.post(WST.U('admin/index/getSubMenus'),{id:menuId},function(data,textStatus){
		 ...
		 var html = [];
		 html.push('<div id="wst-layout-'+menuId+'" style="width:100%;">'); 
...
//	     $("#wst-layout-1").height($("#wst-layout-1").height()+80);
...
$(function (){   
    ...
    $('#wst-tabs .l-tab-links > ul').css('left','227px');
    initTabMenus(tabId.replace('wst-tab-',''));
    $('.l-tab-content').height(WST.pageHeight()-85);
    $('.l-tab-content-item').height(WST.pageHeight()-85);
    $('.wst-iframe').each(function(){
    	$(this).height(WST.pageHeight()-85);
    });
    $('.wst-accordion').each(function(){
    	liger.get($(this).attr('id')).setHeight(WST.pageHeight()-103);
