...
function userGridInit(){
	...
	ugrid = $("#userGrid").ligerGrid({
		url:WST.U('admin/replenishment/pageIndex'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',width: 111,iisSort: false,render: function (rowdata, rowindex, value){
	        	var h="";
	        	if(rowdata["status"]==1){
	        		h=h+'<a class="bianji" href="javascript:jujue(1,'+rowdata["buhuo_id"]+')">同意</a><a class="shanchux" href="javascript:jujue(2,'+rowdata["buhuo_id"]+'">拒绝</a>';
	        	}else if(rowdata["status"]==2){
	        		h=h+'<a class="bianji" href="javascript:getForEdit(1,'+rowdata["buhuo_id"]+')">编辑</a>';
...
function getForEdit(se,id){
	...
	$("#wangdian2").hide();
     ...
     $.post(WST.U('admin/replenishment/get_goods_info'),{id:id},function(data,textStatus){
           layer.close(loading);
           var json = WST.toAdminJson(data);
           
           if(json.buhuo_id){
           		WST.setValues(json);
           		toEdit(se,json.buhuo_id);
           }else{
           		WST.msg(json.msg,{icon:2});
           }
    });
...
//      },
       valid: function(form){
	                ...
	                if(params.wangdian==1&&yy==""){
		                ...
		                var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
		           		$.post(WST.U('admin/replenishment/'+((se==0)?"add":"done_buhuo")),params,function(data,textStatus){
		           			  layer.close(loading);
		           			  var json = WST.toAdminJson(data);
		           			  if(json.status=='1'){
		           			    	WST.msg("操作成功",{icon:1});
...
function jujue(flag,id){
	$.ajax({
		url:WST.U('admin/replenishment/do_buhuo'),
	...
	$("#wangdian2").change(function(){
		...
		if(uu>0){
			if($("#wangdian").val()==1){
				...
				$.ajax({
					type:'post',
					url:WST.U('admin/Replenishment/get_buhuo_list'),
					data:data,
					dataType:'json',
					success:function(data){
//						console.log(data);
						if(data.length>0){
				        	 ...
				        	 for (var i = data.length - 1; i >= 0; i--) {
				        	 	tr=tr+"<tr><td><input class='dsb' type='checkbox' value='" +data[i].tuihuo_id+"'/></td>";
				        	 	tr=tr+"<td class='bianhao'>"+data[i].tuihuo_time+"</td><td>"+data[i].tuihuo_goodsName+"</td><td>"+data[i].tuihuo_remark+"</td><td>"+data[i].tuihuo_do_num+"</td>";
				        	 	tr=tr+"<td><input type='text' class='shuliang' /><span style='display:none;' class='yuanb' >"+data[i].tuihuo_id+","+data[i].tuihuo_spec+","+data[i].tuihuo_goodsName+"</span></td></tr>"
