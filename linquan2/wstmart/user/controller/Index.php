...
class Index extends Base{ 
    ...
    */
    public function item(){
      ...
      $info=db('mb_special_item')->where('special_id',$item_id)->where('dataFlag',1)->order('item_sort asc')->select();
      $arr= $this->index_item($info);      
      if(!empty($arr['goods'])){
    ...
    */
    public function coupon(){
      ...
      $userinfo=db('users')->where('loginName',$user)->find();
      $time=time();
      $coupon=db('discount')->where('discount_shopId',1)->where('discount_startDate','<',$time)->where('discount_endDate','>',$time)->where('discount_delete',1)->field('discount_id,discount_endDate,discount_name,discount_img,discount_flag,discount_value,discount_consume')->select();
